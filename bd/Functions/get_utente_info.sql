...
RETURNS TABLE(hashed_id VARCHAR, email VARCHAR, nome_completo VARCHAR, sexo VARCHAR, morada VARCHAR, data_nascimento VARCHAR, distrito VARCHAR, concelho VARCHAR, freguesia VARCHAR, naturalidade VARCHAR, pais_nacionalidade VARCHAR, tipo_documento_identificacao BIGINT, numero_de_documento_de_identificacao VARCHAR, numero_utente_saude VARCHAR, numero_identificacao_fiscal VARCHAR, numero_seguranca_social VARCHAR, documento_validade VARCHAR, numero_telemovel VARCHAR, nome_entidade_responsavel VARCHAR) AS $$
BEGIN
    RETURN QUERY
